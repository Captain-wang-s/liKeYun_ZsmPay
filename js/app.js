...
createApp({
    setup() {
        ...
        const showPayTips = ref(urlParams.has('order_num')); // 支付提醒
        const showQRCode = ref(urlParams.has('order_num')); // 是否展示二维码
        const showCountdown = ref(urlParams.has('order_num')); // 是否展示倒计时
        const qrcodeStatusText = ref(''); // 二维码状态文案
        const isQRCodeExpired = ref(false); // 二维码是否过期
        const countdown = ref(120); // 倒计时分钟
        ...
        function createOrder(type) {
            axios.get('server/createOrder.php', {
                params: {
                    ...
                    order_id: orderNum.value // 传递A站点的订单号
                }
            })
            .then(response => {
        ...
        function startPolling() {
            pollingInterval = setInterval(() => {
                ...
                .then(response => {
                    ...
                    if (data.code === 0) {
                        // 已支付
                        orderQRCode.value = 'img/wxpay-logo.png';
                        showCountdown.value = false;
        ...
        // 如果有订单号参数，直接展示订单信息
        if (urlParams.has('order_num')) {
            orderQRCode.value = 'img/qrcode.png'; // 假设二维码图片路径
            startCountdown();
            startPolling();
        }
